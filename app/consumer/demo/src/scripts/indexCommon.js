...
var calculatorMain = (function(self) {
    ...
    self.codeNumber = function() {
            var numberMain = 0;
            $('#winColumnMain li input[data-num="calculator-num"]').each(function() {
...
(function meteorAppend() {
    ...
    $('.meteorShower-star').each(function() {
        $(this).addClass(classArray[Math.floor(Math.random() * 3)]);
    });
    $('.meteorShower').each(function() {
        $(this).css('left', parseInt(1.5 * width * Math.random(), 10) - 0.5 * width + 'px');
    });
    var animation = function(){
        for (var i = 0; i < $('.meteorShower-star').length; i++) {
            $('.meteorShower-star').eq(i).css({ 'animation-delay': getRandom(map_speed) + 'ms', 'animation-duration': getRandom(map_speed) + 'ms' });
        }
    };
